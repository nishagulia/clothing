...
const validateForm = () => {
    if(!productName.value.length){
        ...
        return showAlert('select size');
    }else if(!actualPrice.value.length || !discount.value.length ||!sellingPrice.value.length){
        return showAlert("you must add price");
    }else if(stock.value < 20){
        return showAlert('you must have at least 20 item in stock')
    }else if(!tags.value.length){
        return showAlert('enter few tags to help rank');
    }else if(!tac.checked){
        return showAlert('you must agree to trems and cond..');
    }
    return true;
}
...
// exiting product details handle
