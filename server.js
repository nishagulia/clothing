...
var serviceAccount = require("./e-commerce-website-47be2-firebase-adminsdk-bdppm-91f5dfa7f8.json");
...
app.post('/signup', (req, res) => {
    ...
    if(name.length <3){
        ...
        return res.json({'alert':'enter your phnoe number'});
...
app.post('/login', (req, res) => {
    ...
    db.collection('users').doc(email).get()
    .then(user => {
        if(!user.exists){ //is email does not exists
...
app.post('/add-product', (req, res) => {
    ...
    if(!draft){
        if(!name.length){
            ...
            return res.json({'alert':'select size'});
        }else if(!actualPrice.length || !discount.length ||!sellPrice.length){
            return res.json({'alert':"you must add price"});
        }else if(stock < 20){
            return res.json({'alert':'you must have at least 20 item in stock}'})
        }else if(!tags.length){
            return res.json({'alert':'enter few tags to help rank'});
        }else if(!tac){
            return res.json({'alert':'you must agree to trems and cond..'});
        }
    }
            ...
            <html lang="en">
                ...
                <body>
                    ...
                    <div>
                        ...
                        <button class="btn">check ststus</button>
                    </div>
